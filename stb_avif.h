...
/* Internal scalar AV1 decoder (replaces legacy simplified decoder) */
...
/* -------------------------------------------------------------------------- */
/* SCALAR AV1 DECODER WITH RECON HOOKS                                         */
/* -------------------------------------------------------------------------- */

static int stb_avif_decode_frame_scalar(struct stb_av1_tile_context *tc,
                                        const unsigned char *av1_data, size_t av1_size)
{
    /* Parse OBU stream to get sequence and frame headers */
    struct stb_av1_internal_stream stream;
    struct stb_av1_seqhdr seq;
    struct stb_av1_framehdr fh;
    int r;

    stb_av1_internal_stream_init(&stream, av1_data, av1_size);

    /* Parse sequence header */
    r = stbv_av1_obu_parse_seqhdr(&stream, &seq);
    if (r < 0) return -1;

    /* Parse frame header */
    r = stbv_av1_obu_parse_framehdr(&stream, &fh, &seq);
    if (r < 0) return -2;

    /* Verify dimensions match tile context */
    if (tc->frame_width != fh.frame_width || tc->frame_height != fh.frame_height)
        return -3;

    /* Allocate leaf state arrays */
    int frame_w4 = ((fh.frame_width + 7) >> 3) << 1;
    int frame_h4 = ((fh.frame_height + 7) >> 3) << 1;
    int frame_w8 = (fh.frame_width + 7) >> 3;
    int frame_h8 = (fh.frame_height + 7) >> 3;

    /* Above/left context arrays (frame-wide, in 4x4 units) */
    stbv_u8 *above_mode = (stbv_u8 *)stb_avif_calloc(frame_w4, 1);
    stbv_u8 *left_mode = (stbv_u8 *)stb_avif_calloc(frame_h4, 1);
    stbv_u8 *above_tx = (stbv_u8 *)stb_avif_calloc(frame_w4, 1);
    stbv_u8 *left_tx = (stbv_u8 *)stb_avif_calloc(frame_h4, 1);
    stbv_u8 *above_res = (stbv_u8 *)stb_avif_calloc(frame_w4, 1);
    stbv_u8 *left_res = (stbv_u8 *)stb_avif_calloc(frame_h4, 1);
    stbv_u8 *above_cre[2] = { (stbv_u8 *)stb_avif_calloc(frame_w8, 1), (stbv_u8 *)stb_avif_calloc(frame_w8, 1) };
    stbv_u8 *left_cre[2] = { (stbv_u8 *)stb_avif_calloc(frame_h8, 1), (stbv_u8 *)stb_avif_calloc(frame_h8, 1) };
    stbv_u8 *above_skip = (stbv_u8 *)stb_avif_calloc(frame_w4, 1);
    stbv_u8 *left_skip = (stbv_u8 *)stb_avif_calloc(frame_h4, 1);
    stbv_u8 *above_pal_sz = (stbv_u8 *)stb_avif_calloc(frame_w4, 1);
    stbv_u8 *left_pal_sz = (stbv_u8 *)stb_avif_calloc(frame_h4, 1);
    stbv_u8 *above_pal_uv = (stbv_u8 *)stb_avif_calloc(frame_w8, 1);
    stbv_u8 *left_pal_uv = (stbv_u8 *)stb_avif_calloc(frame_h8, 1);
    stbv_u16 *above_pal[2] = { (stbv_u16 *)stb_avif_calloc(frame_w4, 8), (stbv_u16 *)stb_avif_calloc(frame_w4, 8) };
    stbv_u16 *left_pal[2] = { (stbv_u16 *)stb_avif_calloc(frame_h4, 8), (stbv_u16 *)stb_avif_calloc(frame_h4, 8) };

    if (!above_mode || !left_mode || !above_tx || !left_tx ||
        !above_res || !left_res || !above_cre[0] || !left_cre[0] ||
        !above_cre[1] || !left_cre[1] || !above_skip || !left_skip ||
        !above_pal_sz || !left_pal_sz || !above_pal_uv || !left_pal_uv ||
        !above_pal[0] || !left_pal[0] || !above_pal[1] || !left_pal[1])
        return -4;

    /* Leaf state arrays struct */
    struct stbv_av1_leaf_state_arrays arrays = {
        .above_mode = above_mode, .above_mode_n = frame_w4,
        .left_mode = left_mode, .left_mode_n = frame_h4,
        .above_tx = above_tx, .above_tx_n = frame_w4,
        .left_tx = left_tx, .left_tx_n = frame_h4,
        .above_res = above_res, .above_res_n = frame_w4,
        .left_res = left_res, .left_res_n = frame_h4,
        .above_cre = { above_cre[0], above_cre[1] },
        .above_cre_n = { frame_w8, frame_w8 },
        .left_cre = { left_cre[0], left_cre[1] },
        .left_cre_n = { frame_h8, frame_h8 },
        .above_skip = above_skip, .above_skip_n = frame_w4,
        .left_skip = left_skip, .left_skip_n = frame_h4,
        .above_pal_sz = above_pal_sz, .above_pal_sz_n = frame_w4,
        .left_pal_sz = left_pal_sz, .left_pal_sz_n = frame_h4,
        .above_pal_uv = above_pal_uv, .above_pal_uv_n = frame_w8,
        .left_pal_uv = left_pal_uv, .left_pal_uv_n = frame_h8,
        .above_pal = { above_pal[0], above_pal[1] },
        .left_pal = { left_pal[0], left_pal[1] },
        .above_pal_n = frame_w4, .left_pal_n = frame_h4,
    };
    ...
    struct stb_avif_recon_ctx {
        ...
        stbv_i32 cf[64 * 64];
        stbv_u8 pred_scratch[64 * 64];
        int current_bx4, current_by4;
        int current_bw4, current_bh4;
        int current_cbw4, current_cbh4;
        int has_chroma;
        int uv_tx;
        int y_mode, uv_mode;
        int y_angle;
    ...
    recon.layout = tc->sh ? tc->sh->layout : 0;
    recon.ss_hor = recon.layout == STB_AV1_LAYOUT_I420 || recon.layout == STB_AV1_LAYOUT_I422;
    recon.ss_ver = recon.layout == STB_AV1_LAYOUT_I420;
    recon.frame_width = tc->frame_width;
    recon.frame_height = tc->frame_height;
    recon.pixel_max = tc->pixel_max;
    recon.seq = tc->sh;
    recon.frame = tc->fh;

    /* ---- Recon callbacks implementation ---- */
    static void recon_block_info(void *ud,
                                 int intra, int bs, int bx4, int by4,
                                 int has_chroma, int cbw4, int cbh4,
                                 int uv_tx, int tx0,
                                 int pal_sz_y, int pal_sz_uv,
                                 int skip)
    {
        struct stb_avif_recon_ctx *ctx = (struct stb_avif_recon_ctx *)ud;
        if (!ctx) return;
        ctx->current_bx4 = bx4;
        ctx->current_by4 = by4;
        ctx->current_bw4 = stbv_av1_block_dimensions[bs][0];
        ctx->current_bh4 = stbv_av1_block_dimensions[bs][1];
        ctx->current_cbw4 = cbw4;
        ctx->current_cbh4 = cbh4;
        ctx->has_chroma = has_chroma;
        ctx->uv_tx = uv_tx;
        ctx->block_skip = skip;
        ctx->pal_sz_y = pal_sz_y;
        ctx->pal_sz_uv = pal_sz_uv;
    }

    static void recon_luma_txb(void *ud, int x4, int y4, int tx, int txtp, int eob,
                               stbv_i32 *cf)
    {
        struct stb_avif_recon_ctx *ctx = (struct stb_avif_recon_ctx *)ud;
        if (!ctx) return;

        int bw4 = ctx->current_bw4;
        int bh4 = ctx->current_bh4;
        int txw4 = stbv_av1_tx_dims[tx].w;
        int txh4 = stbv_av1_tx_dims[tx].h;
        int w = txw4 << 2;
        int h = txh4 << 2;

        /* Inverse transform: coefficients -> residual in pred_scratch */
        stbv_av1_inv_txfm_add8(ctx->pred_scratch, w, cf, tx, txtp);

        /* Intra prediction on top of residual */
        if (ctx->y_mode != STBV_AV1_INTRA_FILTER) {
            /* Build edge buffer from reconstructed neighbors */
            stbv_u8 tl[256 + 128];
            stbv_u8 *edge = tl + 128;
            int x = x4 << 2;
            int y = y4 << 2;
            int maxv = (1 << ctx->bit_depth) - 1;

            /* Top edge */
            if (y > 0) {
                for (int i = 0; i < w; i++)
                    edge[i] = ctx->plane_y[(y - 1) * ctx->stride_y + x + i];
            } else {
                for (int i = 0; i < w; i++) edge[i] = maxv >> 1;
            }
            /* Left edge */
            if (x > 0) {
                for (int i = 0; i < h; i++)
                    edge[-1 - i] = ctx->plane_y[(y + i) * ctx->stride_y + x - 1];
            } else {
                for (int i = 0; i < h; i++) edge[-1 - i] = maxv >> 1;
            }
            /* Top-left corner */
            edge[-1 - w] = (x > 0 && y > 0) ? ctx->plane_y[(y - 1) * ctx->stride_y + x - 1] : (maxv >> 1);

            /* Run intra prediction: pred_scratch = pred + residual */
            int mode = ctx->y_mode;
            int angle = ctx->y_angle;
            int filter_idx = ctx->filter_idx;
            int bd = ctx->bit_depth;

            if (bd == 8) {
                stbv_av1_ipred_run_8(mode, ctx->pred_scratch, w, edge, w, h,
                                     angle, filter_idx, w, h, bd);
            } else {
                stbv_av1_ipred_run_16(mode, (stbv_u16 *)ctx->pred_scratch, w,
                                      (const stbv_u16 *)edge, w, h,
                                      angle, filter_idx, w, h, bd);
            }

            /* Write to output plane */
            for (int i = 0; i < h; i++) {
                stbv_u8 *dst = ctx->plane_y + (y + i) * ctx->stride_y + x;
                for (int j = 0; j < w; j++)
                    dst[j] = ctx->pred_scratch[i * w + j];
            }
        } else {
            /* Filter intra: pred_scratch already contains prediction+residual */
            int x = x4 << 2;
            int y = y4 << 2;
            for (int i = 0; i < h; i++) {
                stbv_u8 *dst = ctx->plane_y + (y + i) * ctx->stride_y + x;
    ...
    static void recon_chroma_txb(void *ud, int pl, int x4, int y4, int tx, int txtp, int eob,
                                 stbv_i32 *cf)
    {
        struct stb_avif_recon_ctx *ctx = (struct stb_avif_recon_ctx *)ud;
        if (!ctx || !ctx->has_chroma) return;

        int txw4 = stbv_av1_tx_dims[tx].w;
        int txh4 = stbv_av1_tx_dims[tx].h;
        int w = txw4 << 2;
        int h = txh4 << 2;
        int x = (x4 >> ctx->ss_hor) << 2;
        int y = (y4 >> ctx->ss_ver) << 2;

        if (ctx->uv_mode == STBV_AV1_INTRA_CFL) {
            /* Simplified CFL: use luma DC from the corresponding luma block */
            int luma_x = x4 << 2;
            int luma_y = y4 << 2;
            int luma_w = ctx->current_bw4 << 2;
            int luma_h = ctx->current_bh4 << 2;
            int luma_dc = 0;
            for (int i = 0; i < luma_h; i++) {
                for (int j = 0; j < luma_w; j++)
                    luma_dc += ctx->plane_y[(luma_y + i) * ctx->stride_y + luma_x + j];
            }
            luma_dc = (luma_dc + (luma_w * luma_h) / 2) / (luma_w * luma_h);

            int base = 1 << (ctx->bit_depth - 1);
            int alpha_u = cf[0];
            int alpha_v = cf[1];
            int dc_u = base + ((alpha_u * (luma_dc - base)) >> 6);
            int dc_v = base + ((alpha_v * (luma_dc - base)) >> 6);
            dc_u = dc_u < 0 ? 0 : dc_u > 255 ? 255 : dc_u;
            dc_v = dc_v < 0 ? 0 : dc_v > 255 ? 255 : dc_v;

            for (int i = 0; i < h; i++) {
                stbv_u8 *u_dst = ctx->plane_u + (y + i) * ctx->stride_u + x;
                stbv_u8 *v_dst = ctx->plane_v + (y + i) * ctx->stride_v + x;
                for (int j = 0; j < w; j++) {
                    u_dst[j] = (stbv_u8)dc_u;
                    v_dst[j] = (stbv_u8)dc_v;
                }
            }
        } else {
            /* Inverse transform to pred_scratch */
            stbv_av1_inv_txfm_add8(ctx->pred_scratch, w, cf, tx, txtp);
            ...
            if (y > 0) {
                for (int i = 0; i < w; i++)
                    edge[i] = ctx->plane_u[(y - 1) * ctx->stride_u + x + i];
            } else {
                for (int i = 0; i < w; i++) edge[i] = maxv >> 1;
            }
            if (x > 0) {
                for (int i = 0; i < h; i++)
                    edge[-1 - i] = ctx->plane_u[(y + i) * ctx->stride_u + x - 1];
            } else {
                for (int i = 0; i < h; i++) edge[-1 - i] = maxv >> 1;
            }
            edge[-1 - w] = (x > 0 && y > 0) ? ctx->plane_u[(y - 1) * ctx->stride_u + x - 1] : (maxv >> 1);

            int mode = ctx->uv_mode;
            int angle = 0;
            int filter_idx = 0;
            int bd = ctx->bit_depth;
            ...
            stbv_u8 *u_dst = ctx->plane_u + y * ctx->stride_u + x;
            stbv_u8 *v_dst = ctx->plane_v + y * ctx->stride_v + x;
            for (int i = 0; i < h; i++) {
                for (int j = 0; j < w; j++) {
                    u_dst[i * ctx->stride_u + j] = ctx->pred_scratch[i * w + j];
                    v_dst[i * ctx->stride_v + j] = ctx->pred_scratch[i * w + j];
                }
            }
        }
    }

    static void recon_luma_pal(void *ud, const stbv_u8 *idx_map, int sz, int bw4, int bh4,
                               const stbv_u16 *pal)
    {
        struct stb_avif_recon_ctx *ctx = (struct stb_avif_recon_ctx *)ud;
        if (!ctx || !ctx->pal_sz_y) return;

        int x = ctx->current_bx4 << 2;
        int y = ctx->current_by4 << 2;
        int w = bw4 << 2;
        int h = bh4 << 2;
        ...
        for (int i = 0; i < h; i++) {
            stbv_u8 *dst = ctx->plane_y + (y + i) * ctx->stride_y + x;
            for (int j = 0; j < w; j++) {
                int idx = idx_map[i * w + j];
                dst[j] = (stbv_u8)(idx < sz ? pal[idx] : 0);
            }
        }
    }

    static void recon_chroma_pal(void *ud, int pl, const stbv_u8 *idx_map, int sz,
                                 int cbw4, int cbh4, const stbv_u16 *pal)
    {
        struct stb_avif_recon_ctx *ctx = (struct stb_avif_recon_ctx *)ud;
        if (!ctx || !ctx->has_chroma || !ctx->pal_sz_uv) return;

        int x = (ctx->current_bx4 >> ctx->ss_hor) << 2;
        int y = (ctx->current_by4 >> ctx->ss_ver) << 2;
        int w = cbw4 << 2;
        int h = cbh4 << 2;

        stbv_u8 *dst = pl == 0 ? ctx->plane_u : ctx->plane_v;
        int stride = pl == 0 ? ctx->stride_u : ctx->stride_v;

        for (int i = 0; i < h; i++) {
            stbv_u8 *row = dst + (y + i) * stride + x;
            for (int j = 0; j < w; j++) {
                int idx = idx_map[i * w + j];
                row[j] = (stbv_u8)(idx < sz ? pal[idx] : 0);
            }
        }
    }
    /* ---- End recon callbacks ---- */

    stbv_av1_leaf_recon recon_cb = {0};
    recon_cb.ud = &recon;
    recon_cb.cf = recon.cf;
    recon_cb.block_info = recon_block_info;
    recon_cb.luma_txb = recon_luma_txb;
    recon_cb.chroma_txb = recon_chroma_txb;
    recon_cb.luma_pal = recon_luma_pal;
    recon_cb.chroma_pal = recon_chroma_pal;
    /* ---- End recon callbacks ---- */

    /* Initialize tile decoder */
    struct stb_av1_tile_decoder td = {0};
    td.msac = stream.msac;  /* Reuse the bitstream reader */
    td.cdf = stream.cdf;    /* Need to init CDFs properly */
    td.state = &state;
    td.seq = &seq;
    td.frame = &fh;
    td.br = tc->br;
    td.qindex_y = fh.base_q_idx;
    td.qindex_u = fh.base_q_idx;
    td.qindex_v = fh.base_q_idx;
    td.plane_y = tc->plane_y;
    td.plane_u = tc->plane_u;
    td.plane_v = tc->plane_v;
    td.stride_y = tc->stride_y;
    td.stride_u = tc->stride_u;
    td.stride_v = tc->stride_v;
    td.bit_depth = tc->bit_depth;

    /* Decode each tile */
    for (int tile_row = 0; tile_row < fh.tile_rows; tile_row++) {
        for (int tile_col = 0; tile_col < fh.tile_cols; tile_col++) {
            td.tile_row = tile_row;
            td.tile_col = tile_col;

            /* Initialize tile bitstream from the OBU stream */
            /* The remaining data in the OBU stream after frame header is tile data */
            td.msac = &stream.msac;
            td.cdf = stream.cdf;

            /* Decode leaf blocks in this tile - iterate over superblocks (64x64 = 16x16 4x4 units) */
            for (int sby4 = tile_row * (frame_h4 / fh.tile_rows);
                 sby4 < (tile_row + 1) * (frame_h4 / fh.tile_rows);
                 sby4 += 16) {
                stbv_av1_leaf_state_reset_row(&state);

                for (int sbx4 = tile_col * (frame_w4 / fh.tile_cols);
                     sbx4 < (tile_col + 1) * (frame_w4 / fh.tile_cols);
                     sbx4 += 16) {
                    /* Decode all blocks in this 64x64 superblock */
                    for (int by4 = sby4; by4 < sby4 + 16 && by4 < frame_h4; by4++) {
                        for (int bx4 = sbx4; bx4 < sbx4 + 16 && bx4 < frame_w4; bx4++) {
                            /* Determine block size at this position from partition */
                            int bs = 13; /* default 64x64; would be overridden by partition data */
                            int bw4 = stbv_av1_block_dimensions[bs][0];
                            int bh4 = stbv_av1_block_dimensions[bs][1];

                            /* Skip if already processed by larger block */
                            if (bx4 > sbx4 && (stbv_av1_block_dimensions[13][0] > 1))
                                continue;

                            stbv_av1_leaf_tx_result out;
                            int r = stbv_av1_decode_leaf_syntax(&td.msac, td.cdf, &state,
                                                                &seq, &fh, bs, bx4, by4,
                                                                &out, &recon_cb);
                            if (r < 0) return -5;

                            /* Handle sub-blocks if partitioned */
                            if (out.tx == STBV_AV1_TX_4X4 && bw4 > 4) {
                                /* Would need to recurse for sub-blocks */
                            }
                        }
                    }
                }
    ...
    /* Free context arrays */
    stb_avif_free_internal(above_mode);
    stb_avif_free_internal(left_mode);
    stb_avif_free_internal(above_tx);
    stb_avif_free_internal(left_tx);
    stb_avif_free_internal(above_res);
    stb_avif_free_internal(left_res);
    stb_avif_free_internal(above_cre[0]);
    stb_avif_free_internal(left_cre[0]);
    stb_avif_free_internal(above_cre[1]);
    stb_avif_free_internal(left_cre[1]);
    stb_avif_free_internal(above_skip);
    stb_avif_free_internal(left_skip);
    stb_avif_free_internal(above_pal_sz);
    stb_avif_free_internal(left_pal_sz);
    stb_avif_free_internal(above_pal_uv);
    stb_avif_free_internal(left_pal_uv);
    stb_avif_free_internal(above_pal[0]);
    stb_avif_free_internal(left_pal[0]);
    stb_avif_free_internal(above_pal[1]);
    stb_avif_free_internal(left_pal[1]);
...
#else
    {
        /* Scalar AV1 decoder using leaf recon hooks */
        extern int stb_avif_decode_frame_scalar(struct stb_av1_tile_context *tc,
                                                 const unsigned char *av1_data, size_t av1_size);
